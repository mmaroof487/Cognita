...
async def sync_repo(tenant_id: UUID, org_id: UUID, owner: str, repo_name: str):
    ...
    async with async_session_factory() as db:
        ...
        stmt = stmt.on_conflict_do_update(
            ...
            set_={"full_name": stmt.excluded.full_name, "last_synced_at": stmt.excluded.last_synced_at}
        )
        ...
        # Insert Developers
        dev_logins = set(c.get("author_login") or "unknown" for c in commits)
        ...
        for c in commits:
            ...
            author_login = c.get("author_login") or "unknown"
            
            c_stmt = insert(CommitEvent).values(
                tenant_id=tenant_id,
                repo_id=repo.id,
                developer_id=dev_map.get(author_login),
                sha=c["sha"],
                message=c["message"],
            ...
            ).on_conflict_do_nothing(index_elements=["tenant_id", "sha", "committed_at"])
        ...
        pr_dev_logins = set(p.get("author_login") or "unknown" for p in prs)
        ...
        for p in prs:
            ...
            author_login = p.get("author_login") or "unknown"
                
            p_stmt = insert(PrEvent).values(
                tenant_id=tenant_id,
                repo_id=repo.id,
                author_id=dev_map.get(author_login),
